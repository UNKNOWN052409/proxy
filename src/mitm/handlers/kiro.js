...
// Debug trace log — written to logs directory (dev only)
const DEBUG_LOG = path.join(LOGS_DIR, "kiro-debug.log");
function dbg(msg) {
  if (!IS_DEV) return;
  try {
...
// Module-level regex for thinking tag detection (performance optimization)
const START_RE = /<thinking>|<think>/i;
...
function extractThinking(text, state) {
  ...
  if (state.inThink && state.thinkBuf) {
    // Safety: prevent memory exhaustion from unbounded thinking buffer (100KB limit)
    const MAX_THINK_BUF = 100 * 1024;
    if (state.thinkBuf.length > MAX_THINK_BUF) {
      dbg(`[WARN] thinkBuf exceeded ${MAX_THINK_BUF} bytes, truncating`);
      state.thinkBuf = state.thinkBuf.slice(-MAX_THINK_BUF);
    }
  ...
  const startMatch = working.match(START_RE);
...
function buildEventStreamFrame(eventType, payload) {
  ...
  // Payload size validation: reject oversized frames (10MB limit)
  const MAX_PAYLOAD_SIZE = 10 * 1024 * 1024;
  if (payloadBuf.length > MAX_PAYLOAD_SIZE) {
    throw new Error(`EventStream frame payload too large: ${payloadBuf.length} bytes (max ${MAX_PAYLOAD_SIZE})`);
  }
  ...
  if (delta.tool_calls) {
    ...
    for (const tc of delta.tool_calls) {
      ...
      if (tc.function?.arguments) {
        ...
        // Safety: validate arguments before buffering (prevent memory issues)
        const args = tc.function.arguments;
        if (typeof args !== 'string') {
          dbg(`[WARN] Invalid tool arguments type: ${typeof args}, skipping fragment`);
          continue;
        }

        // Safety: check fragment size (1MB limit per fragment)
        const MAX_ARG_FRAGMENT = 1024 * 1024;
        if (args.length > MAX_ARG_FRAGMENT) {
          dbg(`[WARN] Tool argument fragment too large: ${args.length} bytes, truncating`);
          const truncated = args.slice(0, MAX_ARG_FRAGMENT);
          frames.push(buildEventStreamFrame("toolUseEvent", {
            input: truncated,
        ...
        dbg(`toolUseEvent fragment: ${args.slice(0, 100)}`);
        frames.push(buildEventStreamFrame("toolUseEvent", {
          input: args,
          name: init?.name || tc.function?.name || "",
          toolUseId: init?.id || tc.id || ""
...
async function intercept(req, res, bodyBuffer, mappedModel) {
  try {
    // Payload size validation: reject oversized requests (10MB limit)
    const MAX_PAYLOAD_SIZE = 10 * 1024 * 1024;
    if (bodyBuffer.length > MAX_PAYLOAD_SIZE) {
      throw new Error(`Request payload too large: ${bodyBuffer.length} bytes (max ${MAX_PAYLOAD_SIZE})`);
    }
...
/**
 * Validate AWS EventStream frame CRC checksums.
 * Returns true if both prelude and message CRCs are valid.
 */
function validateEventStreamCRC(buffer) {
  if (!buffer || buffer.length < 12) return false;

  const totalLen = buffer.readUInt32BE(0);
  if (buffer.length < totalLen) return false; // incomplete frame

  // Validate prelude CRC (covers first 8 bytes)
  const preludeCRC = buffer.readUInt32BE(8);
  const computedPreludeCRC = crc32(buffer.slice(0, 8));
  if (preludeCRC !== computedPreludeCRC) {
    dbg(`[WARN] EventStream prelude CRC mismatch: expected ${preludeCRC}, got ${computedPreludeCRC}`);
    return false;
  }

  // Validate message CRC (covers entire frame except last 4 bytes)
  const messageCRC = buffer.readUInt32BE(totalLen - 4);
  const computedMessageCRC = crc32(buffer.slice(0, totalLen - 4));
  if (messageCRC !== computedMessageCRC) {
    dbg(`[WARN] EventStream message CRC mismatch: expected ${messageCRC}, got ${computedMessageCRC}`);
    return false;
  }
...
// Detect AWS EventStream binary format with CRC validation
function isBinaryEventStream(buffer) {
  ...
  if (!(totalLen > 12 && totalLen < 1000000 && headersLen < totalLen - 12)) {
    return false;
  }

  // Validate CRC checksums to confirm this is a valid EventStream frame
  return validateEventStreamCRC(buffer);
}
