...
The authenticated `/v1/chat/completions` boundary adds a bounded reliability layer before provider dispatch. Requests are queued with high, normal, or low priority and defaults to **12** concurrent routed operations plus a **96-request** pending backlog, so bursts such as ten simultaneous requests are managed rather than dropped. Operators may tune these limits with `GATEWAY_MAX_CONCURRENCY` and `GATEWAY_MAX_QUEUE_SIZE` after measuring their authorized upstream.

Upstream calls use a default **15-second** response deadline (`GATEWAY_UPSTREAM_TIMEOUT_MS`, accepted range 1,000–120,000 ms) because legitimate models may need longer than five seconds to begin and complete a small response. Retryable timeout, transient network, HTTP 408/425/429, and 5xx failures are retried once after the required **five-second** delay (`GATEWAY_RETRY_DELAY_MS`, default 5,000 ms; `GATEWAY_MAX_RETRIES`, default 1). Clients that require deduplication should send an `Idempotency-Key`; the gateway caches the result reference for five minutes and forwards that key to OpenAI-compatible upstreams.
